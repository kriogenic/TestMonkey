...
namespace TestMonkey
{
    public class CardMonkeyMod : BloonsTD6Mod
    {
        public override void OnTowerUpgraded(Tower tower, string upgradeName, TowerModel newBaseTowerModel)
        {
            ...
            if (tower.rootModel.Cast<TowerModel>().name.ToLower().Contains("testmonkey"))
            {
                ...
                foreach (Tower theTower in inRange)
                {
                    ...
                    TowerModel newModel = ModifyWeapon(theTower, newBaseTowerModel);
                    newBaseTowerModel = newModel;
        ...
        public TowerModel ModifyWeapon(Tower takeFromTower, TowerModel giveToTower)
        {
            //If we are a pilot, ace or upgraded dartling\boomerang don't take it
            if (takeFromTower.towerModel.name.ToLower().Contains("helipilot") || takeFromTower.towerModel.name.ToLower().Contains("monkeyace") ||
                Regex.IsMatch(takeFromTower.towerModel.name, "DartlingGunner-4..") || Regex.IsMatch(takeFromTower.towerModel.name, "DartlingGunner-5..") ||
                Regex.IsMatch(takeFromTower.towerModel.name, "BoomerangMonkey-5..")) return giveToTower;
    ...
    //{
    //    public static TowerModel? ModifyWeapon(Tower takeFromTower, TowerModel giveToTower)
    //    {
    //        //If we are a pilot, ace or upgraded dartling\boomerang don't take it
